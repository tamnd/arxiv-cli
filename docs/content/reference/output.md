...
describes what users get. Pick a format with `-o`, or let arxiv choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
arxiv <command> -o table   # aligned columns for reading
arxiv <command> -o jsonl   # one JSON object per line, for piping
arxiv <command> -o json    # a single JSON array
arxiv <command> -o csv     # spreadsheet friendly
arxiv <command> -o tsv     # tab-separated
arxiv <command> -o url     # just the URL column
arxiv <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
arxiv <command> --fields id,title,url
```
...
```bash
arxiv <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
arxiv <command>            # a table, because this is a terminal
arxiv <command> | wc -l    # JSONL, because this is a pipe
```
